...
class MobileNetV1Prune(nn.Module):
    def __init__(self, cfg = None):
        super(MobileNetV1Prune, self).__init__()
        if cfg is None:
            cfg = [32, 32,
                   64, 64,
                   128, 128, 128, 128,
        ...
        self.model = nn.Sequential(
            ...
            nn.Conv2d(cfg[1], cfg[2], kernel_size=1, stride=1, padding=0),
            ...
            nn.Conv2d(cfg[3], cfg[4], kernel_size=1, stride=1, padding=0),
            ...
            nn.Conv2d(cfg[5], cfg[6], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[6]),
            nn.ReLU(inplace=True),

            # dw4 128 256 2
            nn.Conv2d(cfg[6], cfg[7], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(cfg[7]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[7], cfg[8], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[8]),
            nn.ReLU(inplace=True),

            # dw5 256 256 1
            nn.Conv2d(cfg[8], cfg[9], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(cfg[9]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[9], cfg[10], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[10]),
            nn.ReLU(inplace=True),

            # dw6 256 512 2
            nn.Conv2d(cfg[10], cfg[11], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(cfg[11]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[11], cfg[12], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[12]),
            nn.ReLU(inplace=True),

            # dw7 512 512 1
            nn.Conv2d(cfg[12], cfg[13], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(cfg[13]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[13], cfg[14], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[14]),
            nn.ReLU(inplace=True),

            # dw8 512 512 1
            nn.Conv2d(cfg[14], cfg[15], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(cfg[15]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[15], cfg[16], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[16]),
            nn.ReLU(inplace=True),

            # dw9 512 512 1
            nn.Conv2d(cfg[16], cfg[17], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(cfg[17]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[17], cfg[18], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[18]),
            nn.ReLU(inplace=True),

            # dw10 512 512 1
            nn.Conv2d(cfg[18], cfg[19], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(cfg[19]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[19], cfg[20], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[20]),
            nn.ReLU(inplace=True),

            # dw11 512 512 2
            nn.Conv2d(cfg[20], cfg[21], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(cfg[21]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[21], cfg[22], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(cfg[22]),
            nn.ReLU(inplace=True),

            # dw12 512 512 1
            nn.Conv2d(cfg[22], cfg[23], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(cfg[23]),
            nn.ReLU(inplace=True),

            nn.Conv2d(cfg[23], 512, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
        )
    ...
    def __init__(self):
        ...
        def conv_bn(inp, oup, strid):
            return nn.Sequential(
                nn.Conv2d(inp, oup, 3, stride=strid, padding=1),
                nn.BatchNorm2d(oup),
                nn.ReLU(inplace=True)
            )

        def conv_dw(inp, oup, strid):
            return nn.Sequential(
                nn.Conv2d(inp, inp, 3, stride=strid, padding=1, groups=inp),
                nn.BatchNorm2d(inp),
                nn.ReLU(inplace=True),
        ...
        self.model = nn.Sequential(
            ...
            conv_dw(512, 512, 2),     # dw11
            conv_dw(512, 512, 1),     # dw12
        )
...
class tiny_detector(nn.Module):
    ...
    def __init__(self, n_classes, cfg = None):
        ...
        self.base = MobileNetV1Prune(cfg)
